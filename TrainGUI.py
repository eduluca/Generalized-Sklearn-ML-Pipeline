...
class PanZoomWindow(object):
    ...
    def menuChange(self,k,save_file):
        if k == ord('m'):
            print("""
            ...
            if menSel == ord('s'):
                print('Saving...')
                self.export_point_data(save_file)
                # self.write2csv(save_file)
                savename = input("Save as: [Input Name] ")
                rootPath = os.path.join(os.path.dirname(__file__),'trained-bin',savename+'.pkl')
    ...
    def export_point_data(self,save_folder):
        os.chdir(os.path.join(os.path.dirname(__file__),save_folder))
        array = self.img[:,:,0] == 255
        array.astype('int16').tofile(self.IMG_NAME+"_traind.bin")
    'end def'

    def write2csv(self,save_folder):
        savef = os.path.join(os.path.dirname(__file__),save_folder,self.IMG_NAME+"_datainf.csv")
...
def main(window):
    ...
    while k != ord('q') and k != 27 and cv2.getWindowProperty(window.WINDOW_NAME,0) >= 0:
        k = cv2.waitKey(0) 
        window.menuChange(k,"trained-bin")
    'end while'
    # close all open windows
    cv2.destroyAllWindows()
    window.export_point_data("trained-bin")
    window.write2csv("trained-bin")
    return window
'end def'
