...
def float2uint(arr):
    if type(arr[0,0]) == 'numpy.uint64':
        pass
    else:
        intArr = np.multiply(arr,255).astype(np.uint64)
    #endif
    return intArr
...
def uint2float(arr):
    if arr.dtype == 'float32':
        ...
        floatArr = np.divide(arr,255).astype(np.float32)
    return floatArr

def countless(data,factorH,factorW):
    """
    Vectorized implementation of downsampling a 2D 
    image by 2 on each side using the COUNTLESS algorithm.
    ...
    # all the result of striding by 2 and offset by (0,0), (0,1), (1,0), 
    # and (1,1) representing the A, B, C, and D positions from Figure 1.
    factor = (2,2) #(np.int16(factorH),np.int16(factorW))
    for offset in np.ndindex(factor):
        part = data[tuple(np.s_[o::f] for o, f in zip(offset, factor))]
        sections.append(part)
    #endfor
...
if __name__=="__main__":
    ...
    imDir = os.path.abspath(os.path.join(dirname,"..","a_dataGeneration","rawData"))
    image = np.array(cv2.imread(os.path.join(imDir,"C2Hx 2.2 773951-82_Section01_IL_20x_Overlay.tif"))[:,:,2]/255).astype(np.float32)
